...
void TEE_ST7735::fillQuatCircle(uint8_t x0, uint8_t y0, uint8_t r, uint16_t color, uint8_t quat) {
  int16_t f = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
  int16_t x = 0;
  ...
  if (quat & 12) drawVerticalLine(x0, y0, r, color);
  if (quat & 3)  drawVerticalLine(x0, y0-r, r, color);
  ...
  while (x<y) {
    ...
    if (quat & 1)
    {
      drawVerticalLine(x0+x, y0-y, y, color);
      drawVerticalLine(x0+y, y0-x, x, color);
    }
    if (quat & 2)
    {
      drawVerticalLine(x0-x, y0-y, y, color);
    ...
    if (quat & 4)
    {
      drawVerticalLine(x0-x, y0, y, color);
      drawVerticalLine(x0-y, y0, x, color);
    }
    if (quat & 8)
    {
      drawVerticalLine(x0+x, y0, y, color);
